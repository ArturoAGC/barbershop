...
<div class="max-w-xl mx-auto px-4 py-8">
    ...
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-bold text-yellow-500">Editar servicio</h1>
    </div>
    ...
    <form action="{{ route('admin.services.update', $service) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="w-full rounded-lg bg-gray-900 border border-gray-700 text-white px-4 py-2.5 text-sm focus:outline-none focus:border-yellow-600 transition">{{ old('description', $service->description) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', $service->duration_minutes) }}" min="1"
                       class="w-full rounded-lg bg-gray-900 border border-gray-700 text-white px-4 py-2.5 text-sm focus:outline-none focus:border-yellow-600 transition">
            </div>
        </div>
