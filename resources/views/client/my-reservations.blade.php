                ...
                <tr class="hover:bg-gray-900 transition">
                    ...
                    <td class="px-5 py-4">
                        @if ($reservation->payment?->status === 'approved')
                            <span class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-green-900/50 text-green-400 border border-green-700">Pagado</span>
                        @elseif ($reservation->payment && in_array($reservation->payment->status, ['pending', 'in_process']))
                            <span class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-yellow-900/40 text-yellow-400 border border-yellow-700">Procesando</span>
                        @elseif ($reservation->payment?->status === 'rejected')
                            <span class="inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-red-900/40 text-red-400 border border-red-700">Rechazado</span>
                        @else
                            <span class="text-gray-700 text-xs">Sin pago</span>
                        @endif
                    </td>
                    <td class="px-5 py-4 text-right">
                        <div class="flex justify-end gap-3 items-center">
                            @if (in_array($reservation->status, ['pending', 'confirmed']) && $reservation->payment?->status !== 'approved')
                                <form action="{{ route('client.reservations.pay', $reservation) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit"
                                            class="text-yellow-500 hover:text-yellow-400 text-xs font-medium transition">
                                        Pagar anticipo
                                    </button>
                                </form>
                            @endif
